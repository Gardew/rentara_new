...
const TenantProfile = (props) => {
    ...
    const tabs = [
        {
            id: "information",
            title: t('tenantProfile.tabs.information'),
            content: <EditTenant tenant={tenant} />
        },
        {
            id: "leases",
            title: t('tenantProfile.tabs.leaseHistory'),
        ...
        {
            id: "requests",
            title: t('tenantProfile.tabs.maintenanceRequests'),
            content: (<MaintenanceTable maintenanceReports={tenant?.maintenanceRequests} subtitle={t('tenantProfile.maintenanceSubtitle')}/> )
        }
    ]
    ...
    return (
        <div className="">
            <Alert variant="destructive" className="mb-2" hidden={currentUnit}>
                <AiFillWarning/>
                <AlertTitle>
                    {t('tenantProfile.alert.title')}
                </AlertTitle>
                <AlertDescription>
                    {t('tenantProfile.alert.desc')}
                </AlertDescription>
            </Alert>
                        ...
                        <div className="flex gap-2">
                            {
                               currentUnit ? (
                                      ...
                                      <Badge variant="negative" className="p-1">
                                        {t('tenantProfile.noUnit')}
                                      </Badge>
                               )
                            }
                            <Badge variant="purple" className="p-1">
                                {t('tenantProfile.addedOn', { date: dateParser(tenant?.createdAt) })}
                            </Badge>
                        </div>
                    ...
                    <div className="flex flex-col gap-2 ">
                        <div className="text-center text-muted-foreground">
                            ...
                            <Button variant="link" className="pl-2 text-primary">
                                {t('tenantProfile.invite.cta')}
                            </Button>
                        </div>
                        <Button variant="outline"  title="User not registered.">
                            <SendIcon className="w-4 h-4 mr-2" />
                            {t('tenantProfile.buttons.message')}
                        </Button>
                    </div>
